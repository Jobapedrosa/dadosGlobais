...
async function vizualizarInformacoesGlobais() {
    ...
    paragrafo.innerHTML =  `Você sabia que o total total de pessoas no mundo é de <span>${pessoasMundo}</span> bilhões de pessoas e que o  total de pessoas com acesso à educação é de <span>${pessoasComAcessoAEducacao}</span> bilhões? O tempo médio gasto por dia estudando é de <span>${horas} horas </span> e <span>${minutos} </span> e o total de pessoas com educação superior é de <span>${pessoasComEducacaoSuperior}</span> bilhões. <br>Será que esses dados apontam para uma realidade de um mundo esclarecido e com facilidade de acesso à Educação?` 
